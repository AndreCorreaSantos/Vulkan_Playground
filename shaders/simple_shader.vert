...
void main() {
  ...
  //float z = 3*(sin(position[0]+push.time)*cos(position[1]+push.time));
  noisePosition = vec3(position[0],position[2],position[1]);
  if(type == 1.0){
    float z = snoise(vec2(position[0]+push.time/2,position[1]+push.time/2))/5;
