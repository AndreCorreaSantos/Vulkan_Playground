...
void LvePipeline::createGraphicsPipeline(const std::string& vertFilepath,
  ...
  VkPipelineShaderStageCreateInfo
      shaderStages[2];  // vertex and fragment stages for now
...
void LvePipeline::bind(VkCommandBuffer commandBuffer) {
  vkCmdBindPipeline(commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS,
                    graphicsPipeline);  // telling the gpu that this is a
                                        // graphics and not a compute pipeline.
}
...
void LvePipeline::defaultPipelineConfigInfo(PipelineConfigInfo& configInfo) {
  ...
  configInfo.inputAssemblyInfo.topology =
      VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST;  // triangles will be assembled,
                                            // expecting lists of 3 vertices per
                                            // triangle in index buffer
  ...
  configInfo.rasterizationInfo.depthClampEnable =
      VK_FALSE;  // no depth culling for now
  configInfo.rasterizationInfo.rasterizerDiscardEnable = VK_FALSE;
  configInfo.rasterizationInfo.polygonMode = VK_POLYGON_MODE_FILL;
  configInfo.rasterizationInfo.lineWidth = 1.0f;
  configInfo.rasterizationInfo.cullMode =
      VK_CULL_MODE_NONE;  // no culling for now
