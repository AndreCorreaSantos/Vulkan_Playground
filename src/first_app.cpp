...
void FirstApp::run() {  // game run logic
  SimpleRenderSystem simpleRenderSystem{
      lveDevice,
      lveRenderer.getSwapChainRenderPass()};  // create render system and
...
void FirstApp::updateGameObjects(std::vector<LveGameObject>& gameObjects) {
  for (auto& obj : gameObjects) {
    obj.transform.translation =
        obj.transform.velocity * obj.transform.translation;
  }
}

void FirstApp::loadGameObjects() {
  std::shared_ptr<LveModel> lvemeshModel =
      createmeshModel(lveDevice, {.0f, .0f, .0f});
  ...
  std::shared_ptr<LveModel> lvesphereModel =
      createsphereModel(lveDevice, {.0f, .0f, .0f});
