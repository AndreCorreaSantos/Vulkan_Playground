...
std::vector<uint32_t> generateMeshIndices(
    int numpoints) {  // function to generate mesh indices for plane
...
std::vector<uint32_t> generateMeshIndices(
    int sectorCount,
    int stackCount) {  // function to generate mesh indices for sphere
  std::vector<uint32_t> indices;
  int k1, k2;
  for (int i = 0; i < stackCount; ++i) {
...
// temporary helper function, creates a 1x1x1 mesh centered at offset with an
// index buffer
...
std::unique_ptr<LveModel> createsphereModel(LveDevice& device,
  ...
  modelBuilder.vertices =
      generateSphereVertices(radius, pi, sectorCount, stackCount);
  for (auto& v : modelBuilder.vertices) {
    v.position += offset;
  }
